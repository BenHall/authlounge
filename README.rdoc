...
Based on Authlogic is a clean, simple, and unobtrusive ruby authentication solution to use with {couchdb}[http://couchdb.apache.org/].
== Requirements

Rails 2.3.2

{peterpunk-couchrest}[http://github.com/peterpunk/couchrest/tree/master] it has some improvements like uniqueness validator.

How different is from authlogic?

No autenticates_many
No scopes
Couchdb ready ;-)
...
You can find anything you want about Authlogic in the {documentation}[http://authlogic.rubyforge.org], all that you need to do is understand the basic design behind it, but remember, there's no active record here!.


1. <b>Authlogic::Session</b>, your session models that extend Authlogic::Session::Base.
2. <b>Authlogic::ActsAsAuthentic</b>, which adds in functionality to your Couchdb::ExtendedDocument model when you call acts_as_authentic.
...
Copyright (c) 2009 {Pedro Visintin}[http://www.pedrovisintin.com], released under the MIT license
Based on
