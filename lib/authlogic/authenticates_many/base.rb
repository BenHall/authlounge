      ...
      def authenticates_many(name, options = {})
        ...
        class_eval <<-"end_eval", __FILE__, __LINE__
          def #{name}
            ...
            @#{name} ||= Authlogic::AuthenticatesMany::Association.new(#{options[:session_class]}, find_options, #{options[:scope_cookies] ? "self.class.model_name.underscore + '_' + self.send(:id).to_s" : "nil"})
          end
        end_eval
      end
    end
    ::CouchRest::Document.extend(Base) if defined?(::CouchRest)
  end
end